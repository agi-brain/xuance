...
from expocomm_agents import ExpoComm_Agents


def parse_args():
    parser = argparse.ArgumentParser("Example: ExpoComm of XuanCe for SMACv2 environments.")
    parser.add_argument("--config", type=str, default="10gen_protoss")
    parser.add_argument("--test", type=int, default=0)
    parser.add_argument("--benchmark", type=int, default=1)

    args = parser.parse_args()
    args.test_mode = bool(args.test)
    return args
...
class RunnerSMACv2:
    def __init__(self, config):
        ...
        self.agents = ExpoComm_Agents(config=config, envs=self.envs)
        self.config = config
    ...
    def test_episodes(self, test_T, n_test_runs):
        ...
        for i_test in range(n_test_runs):
            running_scores = self.agents.run_episodes(n_episodes=self.n_envs, test_mode=True, close_envs=False)
    ...
    def run(self):
        if self.config.test_mode:
            ...
            while self.agents.current_step <= self.running_steps:
                score = self.agents.run_episodes(n_episodes=self.n_envs, test_mode=False)
    ...
    def benchmark(self):
        ...
        time_start = time.time()
        while self.agents.current_step <= self.running_steps:
            self.agents.run_episodes(n_episodes=self.n_envs, test_mode=False)
            if self.agents.current_step >= self.agents.start_training:
...
if __name__ == "__main__":
    ...
    config_path_basic = os.path.join(main_path, "xuance", "configs", "basic.yaml")
    config_basic = load_yaml(config_path_basic)
    
    script_dir = os.path.dirname(__file__)
    config_path = os.path.join(script_dir, f"expocomm_smacv2_configs/{parser.config}.yaml")
    configs_dict = load_yaml(file_dir=config_path)
    configs_dict = recursive_dict_update(config_basic, configs_dict)
    configs_dict = recursive_dict_update(configs_dict, parser.__dict__)
    configs = argparse.Namespace(**configs_dict)

    runner = RunnerSMACv2(configs)
    if parser.benchmark:
        runner.benchmark()
    else:
        runner.run()
