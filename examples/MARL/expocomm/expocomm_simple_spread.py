...
def parse_args():
    parser = argparse.ArgumentParser("Example of XuanCe: ExpoComm for MPE.")
...
if __name__ == "__main__":
    parser = parse_args()
    configs_dict = load_yaml(file_dir=f"expocomm_mpe_configs/{parser.env_id}.yaml")
    configs_dict = recursive_dict_update(configs_dict, parser.__dict__)
    configs = argparse.Namespace(**configs_dict)

    set_seed(configs.seed)
    envs = make_envs(configs)
    Agents = ExpoComm_Agents(config=configs, envs=envs)
