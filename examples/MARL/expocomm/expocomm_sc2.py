...
def parse_args():
    parser = argparse.ArgumentParser("Example: ExpoComm of XuanCe for SMAC environments.")
...
class RunnerSC2:
    def __init__(self, config):
        ...
        self.agents = ExpoComm_Agents(config=config, envs=self.envs)  # Create the Independent PPO agents.
        self.config = config
    ...
    def run(self):
        if self.config.test_mode:
            ...
            while self.agents.current_step <= self.running_steps:
                score = self.agents.run_episodes(n_episodes=self.n_envs, test_mode=False, close_envs=False)
    ...
    def benchmark(self):
        ...
        while self.agents.current_step <= self.running_steps:
            # train
            self.agents.run_episodes(n_episodes=self.n_envs, test_mode=False, close_envs=False)
            if self.agents.current_step >= self.agents.start_training:
...
if __name__ == "__main__":
    ...
    config_path = os.path.join(script_dir, f"expocomm_sc2_configs/{parser.env_id}.yaml")
