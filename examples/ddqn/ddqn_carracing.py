...
def parse_args():
    parser = argparse.ArgumentParser("Example of XuanCe: Double DQN for Car Recing.")
...
if __name__ == "__main__":
    parser = parse_args()
    configs_dict = get_configs(file_dir="ddqn_configs/ddqn_carracing.yaml")
    configs_dict = recursive_dict_update(configs_dict, parser.__dict__)
    configs = argparse.Namespace(**configs_dict)

    set_seed(configs.seed)
    envs = make_envs(configs)
    Agent = DDQN_Agent(config=configs, envs=envs)
