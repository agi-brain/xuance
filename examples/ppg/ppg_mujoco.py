...
def parse_args():
    parser = argparse.ArgumentParser("Example of XuanCe: PPG for MuJoCo.")
...
if __name__ == "__main__":
    parser = parse_args()
    configs_dict = get_configs(file_dir="ppg_configs/ppg_mujoco.yaml")
    configs_dict = recursive_dict_update(configs_dict, parser.__dict__)
    configs = argparse.Namespace(**configs_dict)

    set_seed(configs.seed)  # Set the random seed.
    envs = make_envs(configs)  # Make the environment.
    Agent = PPG_Agent(config=configs, envs=envs)  # Create the PPG agent.
