...
def parse_args():
    parser = argparse.ArgumentParser("Example: DGN of XuanCe for SMAC environments.")
...
class RunnerSC2:
    def __init__(self, config):
        ...
        self.agents = DGN_Agents(config=config, envs=self.envs)  # Create the Independent PPO agents.
        self.config = config
...
if __name__ == "__main__":
    parser = parse_args()
    configs_dict = get_configs(file_dir=f"dgn_sc2_configs/{parser.env_id}.yaml")
