...
def parse_args():
    parser = argparse.ArgumentParser("Example of XuanCe: COMA for MPE.")
...
if __name__ == "__main__":
    parser = parse_args()
    configs_dict = get_configs(file_dir="coma_mpe_configs/simple_spread_v3.yaml")
    configs_dict = recursive_dict_update(configs_dict, parser.__dict__)
    configs = argparse.Namespace(**configs_dict)

    set_seed(configs.seed)  # Set the random seed.
    envs = make_envs(configs)  # Make the environment.
    Agents = COMA_Agents(config=configs, envs=envs)  # Create the Independent PPO agents.
