...
def parse_args():
    parser = argparse.ArgumentParser("Example of XuanCe: C51 for CartPole.")
...
if __name__ == "__main__":
    parser = parse_args()
    configs_dict = get_configs(file_dir="c51_configs/c51_cartpole.yaml")
    configs_dict = recursive_dict_update(configs_dict, parser.__dict__)
    configs = argparse.Namespace(**configs_dict)

    set_seed(configs.seed)
    envs = make_envs(configs)
    Agent = C51_Agent(config=configs, envs=envs)
