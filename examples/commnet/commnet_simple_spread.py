...
if __name__ == "__main__":
    parser = parse_args()
    configs_dict = get_configs(file_dir=f"commnet_mpe_configs/{parser.env_id}.yaml")
    configs_dict = recursive_dict_update(configs_dict, parser.__dict__)
    configs = argparse.Namespace(**configs_dict)

    set_seed(configs.seed)  # Set the random seed.
    envs = make_envs(configs)  # Make the environment.
    Agents = CommNet_Agents(config=configs, envs=envs)  # Create the Independent PPO agents.
