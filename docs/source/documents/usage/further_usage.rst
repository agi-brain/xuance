...
.. mermaid::
    ...
    sequenceDiagram
        ...
        participant GR as get_runner()
        participant R as RunnerDRL.run(mode)
        participant E as VecEnvs
        participant A as Agent
        participant P as Policy(uses Rep)
        participant B as ReplayBuffer
        participant L as Learner

        U->>GR: algo/env/env_id + parser_args
        GR->>GR: load yaml + merge args<br/>select dl_toolbox
        GR-->>U: runner

        U->>R: runner.run("benchmark"|"train"|"test")
        R->>E: make_envs(config)<br/>Dummy/Subproc VecEnv
        R->>A: REGISTRY_Agents[config.agent](config, envs)

        Note over A: build inside Agent<br/>ReplayBuffer<br/> Representation<br/> Policy(uses Rep)<br/> Learner(1 per algo)

        loop training steps
        A->>P: action(obs, test_mode=False)<br/>+ exploration
        P-->>A: actions
        A->>E: step(actions)
        E-->>A: next_obs,reward,done,info
        A->>B: store(transition)
        ...
        R->>A: test(test_envs) / save_model
        R-->>U: benchmark results (csv/json) + best model
