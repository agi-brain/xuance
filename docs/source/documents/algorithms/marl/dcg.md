...
[
\begin{aligned}
\mu^{ij}_t(a^j) \leftarrow \max_{a^i} \Bigg\{ & \frac{1}{|\mathcal{V}|} f^i(a^i|s_t) + \frac{1}{|\mathcal{E}|} f^{ij}(a^i,a^j|s_t) \\
& + \sum_{\{k,i\} \in \mathcal{E}} \mu^{ki}_t(a^i) - \mu^{ji}_t(a^i) \Bigg\}
\end{aligned}
]
