...
$$
\[
\mu_{t}^{ij}(a^{j}) \leftarrow
\max_{a^{i}}\left\{
  \frac{1}{\lvert \mathcal{V} \rvert}\, f^{i}\!\bigl(a^{i}\mid s_{t}\bigr)
  + \frac{1}{\lvert \mathcal{E} \rvert}\, f^{ij}\!\bigl(a^{i},a^{j}\mid s_{t}\bigr)
  + \sum_{k:\,(k,i)\in\mathcal{E}} \mu_{t}^{ki}(a^{i})
  - \mu_{t}^{ji}(a^{i})
\right\}.
\]
$$
