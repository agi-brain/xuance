...
$$
\mu_{t}^{ij}(a^{j}) \leftarrow \max_{a^{i}} \biggl\{ \frac{1}{|\mathcal{V}|}f^{i}(a^{i}|s_{t}) + \frac{1}{|\mathcal{E}|}f^{ij}(a^{i},a^{j}|s_{t}) + \sum_{k,i\in\mathcal{E}}\mu_{t}^{ki}(a^{i}) - \mu_{t}^{ji}(a^{i}) \biggr\}.
$$
