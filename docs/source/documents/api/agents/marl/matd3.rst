...
.. py:function::
    xuance.tensorflow.agent.mutli_agent_rl.matd3_agents.MATD3_Agents.act(obs_n, test_mode=False)
...
.. py:function::
    xuance.tensorflow.agent.mutli_agent_rl.matd3_agents.MATD3_Agents.train(i_episode)
...
.. py:function::
    xuance.mindspore.agent.mutli_agent_rl.matd3_agents.MATD3_Agents.act(obs_n, test_mode=False)
...
.. py:function::
    xuance.mindspore.agent.mutli_agent_rl.matd3_agents.MATD3_Agents.train(i_episode)
            ...
            class MATD3_Agents(MARLAgents):
                ...
                def __init__(self,
                    ...
                    optimizer = [torch.optim.Adam(policy.parameters_actor, config.learning_rate_actor, eps=1e-5),
                                torch.optim.Adam(policy.critic_net_A.parameters(), config.learning_rate_critic, eps=1e-5),
                                torch.optim.Adam(policy.critic_net_B.parameters(), config.learning_rate_critic, eps=1e-5)]
                    scheduler = [torch.optim.lr_scheduler.LinearLR(optimizer[0], start_factor=1.0, end_factor=self.end_factor_lr_decay,
                                ...
                                torch.optim.lr_scheduler.LinearLR(optimizer[1], start_factor=1.0, end_factor=self.end_factor_lr_decay,
                                                                total_iters=get_total_iters(config.agent_name, config)),
                                torch.optim.lr_scheduler.LinearLR(optimizer[2], start_factor=1.0, end_factor=self.end_factor_lr_decay,
                                                                total_iters=get_total_iters(config.agent_name, config))]
            ...
            class MATD3_Agents(MARLAgents):
                def __init__(self,
                    ...
                    memory = MARL_OffPolicyBuffer(config.n_agents,
                                                  ...
                                                  config.buffer_size,
                                                  config.batch_size)
                    learner = MATD3_Learner(config, policy, optimizer, scheduler, config.device, config.model_dir, config.gamma)
                    super(MATD3_Agents, self).__init__(config, envs, policy, memory, learner, config.log_dir, config.model_dir)
                    self.on_policy = False
