...
.. py:function::
  xuance.tensorflow.agent.policy_gradient.sac_agent.SAC_Agent._action(obs)
...
.. py:function::
  xuance.tensorflow.agent.policy_gradient.sac_agent.SAC_Agent.train(train_steps)
...
.. py:function::
  xuance.tensorflow.agent.policy_gradient.sac_agent.SAC_Agent.test(env_fn, test_episodes)
        ...
        class SAC_Agent(Agent):
            def __init__(self,
                ...
                learner = SAC_Learner(policy,
                                      ...
                                      config.gamma,
                                      config.tau)
                super(SAC_Agent, self).__init__(config, envs, policy, memory, learner, config.log_dir, config.model_dir)

            def _action(self, obs):
                states, dists = self.policy.action(ms.Tensor(obs))
                acts = dists.sample().asnumpy()
                return acts
