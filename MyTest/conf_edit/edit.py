...
def create_watermark(text, width, height, position):
    packet = BytesIO()
    c = canvas.Canvas(packet, pagesize=(width, height))
    # 根据图片设置页边距
    top_margin = 1.5 * cm  # 上边距2厘米
    ...
    if position == 'header':
        ...
        c.drawString(left_margin, height - top_margin ,
                     "Proceedings of the 3rd Conference on Fully Actuated System Theory and Applications")  # 主标题
        c.drawString(left_margin, height - top_margin- 12.2,
                     "May 10-12, 2024, Shenzhen, China")  # 日期和地点
    elif position == 'footer':
        # 页脚的文本和格式设置
        c.drawString(60, height - 80, text)  # 页脚文本位置可以根据需要调整
...
def add_header_footer(input_pdf, output_pdf, header_text, footer_text):
    ...
    for i in range(len(reader.pages)):
        ...
        header_watermark = create_watermark(header_text, width, height, 'header')
        footer_watermark = create_watermark(footer_text, width, height, 'footer')
...
# 使用示例
root_folder = './pdf/'
header_text = "Proceedings of the 3rd Conference on Fully Actuated System Theory and Applications \nMay 10-12, 2024, Shenzhen, China"
footer_text = ''
