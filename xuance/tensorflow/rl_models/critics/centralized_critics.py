...
from xuance.tensorflow import tf, keras, Tensor, Module, ModuleDict
from xuance.tensorflow.utils import AgentGroupedTensor
from xuance.tensorflow.rl_models.heads import ValueHead, QValueHead
from xuance.tensorflow.rl_models.modules import MultiAgentModelOutput, CriticOutput, TwinCriticOutput, RNN_State
...
class CentralizedStateValueCritic(Module):
    ...
    def call(self,
        ...
        critic_input = tf.stack([obs_features[k] for k in self.agent_keys], axis=1)  # batch * N * ...
        if expanded_states is not None:
            critic_input = tf.concat([expanded_states, critic_input], axis=-1)
...
class TwinCentralizedActionValueCritic(Module):
    ...
    def call(self,
        ...
        return TwinCriticOutput(
            ...
            values_1=self.critic_head_1(tf.concat([rep_out_1.embeddings, joint_actions], axis=-1), **kwargs),
