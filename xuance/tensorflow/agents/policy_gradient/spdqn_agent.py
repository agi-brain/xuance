...
class SPDQN_Agent(Agent):
    def __init__(self,
        ...
        super(SPDQN_Agent, self).__init__(envs, policy, memory, learner, writer, device, config.log_dir, config.model_dir)
    ...
    def train(self, train_steps=10000):
        ...
        for step in tqdm(range(train_steps)):
            ...
            if terminal == True:
                self.writer.add_scalar("returns-episode", np.sum(scores), episodes)
                self.writer.add_scalar("returns-step", np.sum(scores), step)
                scores = 0
                returns = 0
                episodes += 1
