...
class VDAC_Learner(IAC_Learner):
    def __init__(self, *args, **kwargs):
        super(VDAC_Learner, self).__init__(*args, **kwargs)
        self.use_global_state = True if self.config.mixer == "QMIX" else getattr(self.config, "use_global_state", False)
    ...
    def forward_fn(self, **kwargs):
        ...
        if self.use_global_state:
            batch.global_states = kwargs["global_states"]
        ...
        with tf.GradientTape() as tape:
            ...
            for group, n_agents in self.n_group_agents.items():
                ...
                advantages = tf.reshape(batch.advantages.packed(group), [-1])
                advantages = tf.stop_gradient(advantages)

                actor_loss.append(-tf.reduce_sum((advantages * log_pi) * mask_values) / tf.reduce_sum(mask_values))
    ...
    def update(self, sample):
        ...
        if self.use_global_state:
            inputs_learn["global_states"] = batch.global_states
