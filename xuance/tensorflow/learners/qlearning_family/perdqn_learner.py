...
class PerDQN_Learner(Learner):
    ...
    def learn(self, *inputs):
        if self.distributed_training:
            td_error, predictQ, loss = self.model.mirrored_strategy.run(self.forward_fn, args=inputs)
            return (self.model.mirrored_strategy.reduce(tf.distribute.ReduceOp.SUM, td_error, axis=None),
    ...
    def update(self, **samples):
        ...
        info.update(self.callback.on_update_end(self.iterations, model=self.model, info=info,
                                                predictQ=predictQ, td_error=td_error, loss=loss))
