...
class MFAC_Learner(IPPO_Learner):
    ...
    def update(self, sample):
        ...
        values_pred_dict = self.model.get_values(observations=obs, mean_actions=act_mean,
    ...
    def update_rnn(self, sample):
        ...
        if self.use_global_state:
            ...
            _, values_pred_dict = self.model.get_values(states=state,
                                                        observations=obs, mean_actions=act_mean,
                                                        agent_indices=agent_indices,
                                                        rnn_states=rnn_states_critic).values
        else:
            _, values_pred_dict = self.model.get_values(observations=obs, mean_actions=act_mean,
                                                        agent_indices=agent_indices,
                                                        rnn_states=rnn_states_critic).values
        if not self.agent_grouping.full_independent:
            values_pred_dict = self.packed_tensor(values_pred_dict)
