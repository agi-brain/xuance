...
class IDDPG_Learner(LearnerMAS):
    ...
    def update(self, sample):
        ...
        rewards = {k: Tensor(sample['rewards'][k]).to(self.device) for k in self.model_keys}
        terminals = {k: Tensor(sample['terminals'][k]).float().to(self.device) for k in self.model_keys}
        agent_mask = {k: Tensor(sample['agent_mask'][k]).float().to(self.device) for k in self.model_keys}
        IDs = None
