...
class ITD3_Learner(LearnerMAS):
    def __init__(self,
                 ...
                 policy: nn.Module,
                 callback):
        super(ITD3_Learner, self).__init__(config, model_keys, agent_keys, policy, callback)
        self.optimizer = {
    ...
    def update(self, sample):
        ...
        q_eval_A, q_eval_B, _ = self.policy.Qpolicy(observation=obs, actions=actions_eval, agent_ids=IDs)
        _, next_actions = self.policy.Atarget(next_observation=obs_next, agent_ids=IDs)
        q_next = self.policy.Qtarget(next_observation=obs_next, next_actions=next_actions, agent_ids=IDs)
        ...
        if self.iterations % self.actor_update_delay == 0:
            ...
            for key in self.model_keys:
                mask_values = agent_mask[key]
                _, _, q_policy = self.policy.Qpolicy(observation=obs, actions=actions_eval, agent_ids=IDs,
                                                     agent_key=key)
    ...
    def update_rnn(self, sample):
        ...
        q_eval_A, q_eval_B, _ = self.policy.Qpolicy(observation=obs_t, actions=actions, agent_ids=IDs_t,
                                                    rnn_hidden=rnn_hidden_critic)
        q_next = self.policy.Qtarget(next_observation=obs, next_actions=actions_next, agent_ids=IDs,
                                     rnn_hidden=rnn_hidden_critic)
        ...
        if self.iterations % self.actor_update_delay == 0:
            ...
            for key in self.model_keys:
                mask_values = agent_mask[key] * filled
                _, _, q_policy = self.policy.Qpolicy(observation=obs_t, actions=actions, agent_ids=IDs_t,
