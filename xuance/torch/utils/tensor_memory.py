...
class EpisodeBuffer:
    ...
    def sample(self, lookup_step=None, idx=None) -> Dict[str, Tensor]:
        obs = Tensor(self.obs)
...
class DummyOnPolicyBuffer(Buffer):
    ...
    def __init__(self,
        ...
        super(DummyOnPolicyBuffer, self).__init__(observation_space, action_space, auxiliary_shape, n_envs,
                                                  self.buffer_size)
    ...
    def clear(self):
        ...
        self.rewards = create_tensor_memory((), self.n_envs, self.n_size)
        self.returns = create_tensor_memory((), self.n_envs, self.n_size)
        self.values = create_tensor_memory((), self.n_envs, self.n_size)
        self.terminals = create_tensor_memory((), self.n_envs, self.n_size)
        self.advantages = create_tensor_memory((), self.n_envs, self.n_size)
        self.auxiliary_infos = create_tensor_memory(self.auxiliary_shape, self.n_envs, self.n_size)
...
class DummyOnPolicyBuffer_Atari(DummyOnPolicyBuffer):
    ...
    def clear(self):
        self.ptr, self.size = 0, 0
        self.observations = create_tensor_memory(space2shape(self.observation_space), self.n_envs, self.n_size, torch.uint8)
        self.actions = create_tensor_memory(space2shape(self.action_space), self.n_envs, self.n_size)
        self.rewards = create_tensor_memory((), self.n_envs, self.n_size)
        self.returns = create_tensor_memory((), self.n_envs, self.n_size)
        self.values = create_tensor_memory((), self.n_envs, self.n_size)
...
class DummyOffPolicyBuffer(Buffer):
    ...
    def __init__(self,
                 ...
                 buffer_size: int,
                 batch_size: int):
        super(DummyOffPolicyBuffer, self).__init__(observation_space, action_space, auxiliary_shape, n_envs,
                                                   buffer_size)
    ...
    def sample(self, batch_size=None):
        ...
        env_choices = torch.random.choice(self.n_envs, bs)
        step_choices = torch.random.choice(self.size, bs)
...
class RecurrentOffPolicyBuffer(Buffer):
    ...
    def sample(self):
        ...
        episode_choices = torch.random.choice(self.memory, self.batch_size)
        ...
        if length_min > self.lookup_length:
            for episode in episode_choices:
                start_idx = torch.random.randint(0, len(episode) - self.lookup_length + 1)
            ...
            for episode in episode_choices:
                start_idx = torch.random.randint(0, len(episode) - length_min + 1)
        ...
        samples_dict = {
            'obs': Tensor(obs_batch),
            'actions': Tensor(act_batch),
            'rewards': Tensor(rew_batch),
            'terminals': Tensor(terminal_batch),
...
class PerOffPolicyBuffer(Buffer):
    ...
    def __init__(self,
        ...
        self._max_priority = torch.ones((n_envs))
    ...
    def sample(self, beta):
        env_choices = Tensor(range(self.n_envs)).repeat(int(self.batch_size / self.n_envs))
        step_choices = torch.zeros((self.n_envs, int(self.batch_size / self.n_envs)))
        weights = torch.zeros((self.n_envs, int(self.batch_size / self.n_envs)))
        ...
        for i in range(self.n_envs):
            ...
            weights[i] = Tensor(weights_)
        step_choices = step_choices.astype(torch.int64)
...
class DummyOffPolicyBuffer_Atari(DummyOffPolicyBuffer):
    ...
    def clear(self):
        self.observations = create_tensor_memory(space2shape(self.observation_space), self.n_envs, self.n_size, torch.uint8)
        self.next_observations = create_tensor_memory(space2shape(self.observation_space), self.n_envs, self.n_size, torch.uint8)
        self.actions = create_tensor_memory(space2shape(self.action_space), self.n_envs, self.n_size)
        self.auxiliary_infos = create_tensor_memory(self.auxiliary_shape, self.n_envs, self.n_size)
        self.rewards = create_tensor_memory((), self.n_envs, self.n_size)
        self.terminals = create_tensor_memory((), self.n_envs, self.n_size)
...
class SequentialReplayBuffer(Buffer):
    ...
    def __init__(self,
        ...
        assert buffer_size % self.n_envs == 0, "buffer_size must be divisible by the number of envs (parallels)"
        self.n_size = buffer_size // self.n_envs
        self.obs = create_tensor_memory(space2shape(self.observation_space), self.n_envs, self.n_size)
        self.acts = create_tensor_memory(space2shape(self.action_space), self.n_envs, self.n_size)
        self.rews = create_tensor_memory((), self.n_envs, self.n_size)
        self.terms = create_tensor_memory((), self.n_envs, self.n_size)
        self.truncs = create_tensor_memory((), self.n_envs, self.n_size)
        self.is_first = create_tensor_memory((), self.n_envs, self.n_size)
    ...
    def store(self, obs, acts, rews, terms, truncs, is_first):
        ...
        """
        store_tensor_element(obs, self.obs, self.ptr)
        store_tensor_element(acts, self.acts, self.ptr)
        store_tensor_element(rews, self.rews, self.ptr)
        store_tensor_element(terms, self.terms, self.ptr)
        store_tensor_element(truncs, self.truncs, self.ptr)
        store_tensor_element(is_first, self.is_first, self.ptr)
        self.ptr = (self.ptr + 1) % self.n_size
        self.size = min(self.size + 1, self.n_size)
    ...
    def sample(self, seq_len: int):
        ...
        Args:
            seq_len (int)
        Returns:
            Dict[str, Tensor]: the sampled dictionary with a shape of
        ...
        indices = torch.arange(0, first_range_end)
        if self.size == self.n_size:
            second_range_end = self.ptr + seq_len if first_range_end <= 0 else self.n_size
            indices = torch.concatenate([
                indices,
                torch.arange(self.ptr, second_range_end)
            ])
        li = []
        for _ in range(self.n_envs):
            start = torch.random.choice(indices, size=self.batch_size).reshape(-1, 1)  # (batch, 1)
            seq_arange = torch.arange(seq_len, dtype=torch.int32).reshape(1, -1)  # (1, seq)
            idxes = (start + seq_arange) % self.n_size  # (batch, seq)
            li.append(torch.swapaxes(idxes, 0, 1))  # (seq, batch)
        idxes = torch.stack(li)  # (envs, seq, batch)
        envs = torch.broadcast_to(torch.arange(self.n_envs)[:, None, None], idxes.shape)  # (env, seq, batch)
