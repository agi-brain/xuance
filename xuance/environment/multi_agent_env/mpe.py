...
class MPE_Env(RawMultiAgentEnv):
    ...
    def __init__(self, config):
        ...
        self.continuous_actions = continuous
        ...
        self.action_space = {k: self.env.action_space(k) for k in self.env.agents}
        self.observation_space = {k: self.env.observation_space(k) for k in self.env.agents}
        self.agents = self.env.agents
        self.num_agents = len(self.agents)
        self.groups = self.get_handles()
        self.agent_ids = [self.get_ids(h) for h in self.groups]
        self.n_agents = [self.get_num(h) for h in self.groups]
