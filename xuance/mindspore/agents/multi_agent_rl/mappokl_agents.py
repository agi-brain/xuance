...
class MAPPO_KL_Agents(MARLAgents):
    def __init__(self,
        ...
        if config.state_space is not None:
            ...
            config.dim_state, state_shape = None, None
        memory = MARL_OnPolicyBuffer_MindSpore(state_shape, config.obs_shape, config.act_shape, config.rew_shape,
                                               config.done_shape, envs.num_envs, config.nsteps, config.nminibatch,
                                               config.use_gae, config.use_advnorm, config.gamma, config.lam,
                                               envs.action_space[envs.agent_keys[0][0]].n)
        learner = MAPPO_KL_Learner(config, policy, optimizer, scheduler, writer, config.modeldir, config.gamma)
        ...
        super(MAPPO_KL_Agents, self).__init__(config, envs, policy, memory, learner, writer,
    ...
    def act(self, obs_n, episode, test_mode, state=None, noise=False):
        ...
        return acts.asnumpy(), act_probs.asnumpy(), vs.asnumpy()
    ...
    def value(self, obs, state):
        ...
        values = self.expand_dims(
            ops.broadcast_to(self.policy.value_tot(values_n, global_state=state).view(-1, 1), (-1, self.n_agents)), -1)
