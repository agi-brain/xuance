...
class Scenario(BaseScenario):
    ...
    def observation_noise(self, z):
        obs_noise_cov = np.array(
            [
                [
                    self.sensor_r_sd * self.sensor_r_sd,
                    0.0,
                ],  # z[0]/self.sensor_r * self.sensor_r_sd, 0.0],
    ...
    def make_world(self, args: argparse.Namespace) -> World:
        ...
        self.targetA = np.concatenate(
            (
                np.concatenate((np.eye(2), self.sampling_period * np.eye(2)), axis=1),
                [[0, 0, 1, 0], [0, 0, 0, 1]],
            )
        )
        self.target_noise_cov = METADATA["const_q"] * np.concatenate(
            (
                np.concatenate(
                    (
                        self.sampling_period**3 / 3 * np.eye(2),
        ...
        world.scripted_agents_goals = [
            Landmark() for i in range(self.num_scripted_agents_goals)
        ]
